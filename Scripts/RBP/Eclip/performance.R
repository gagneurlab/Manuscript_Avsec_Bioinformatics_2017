...
cols <- c(col_glmnet, col_relu_shallow, mypal[2], col_concise_deep, col_relu_deep)
## ---------------------------------
...
q_auc <- ggplot(dtb_auc, aes(x = Method, y = auc)) +
  ...
  geom_signif(comparisons = list(c("DeepNN", "DeepNN_pos_gam"),
                                 c("DeepNN_pos_gam", "DeepNN_pos_relu")
                                 ),
...
save_plot(file.path(plt_dir, "fig4.pdf"), plt_ab,
          ncol = 1,
          nrow = 2,
...
save_plot(file.path(plt_dir, "fig4.png"), plt_ab,
          ncol = 1,
          nrow = 2,
          base_aspect_ratio = 2.2,
          base_height=4,
...
plot_grid(qpl1_auprc, qpl2_auprc)
